...
```sh
dsh plugin --profile web add dsh-interactive-reader@0.9.1
```
...
Tool calls that changed files carry `+A -R` statistics and open a per-file diff panel, and third-party tool cards registered in the host's `tool.call.toolview` slot render inside the reading view. Settings → **Interactive Reader** also holds the translucent frosted glass toggle and a three-stop auto-fold intensity.
...
`lib/dsh-interactive-reader.js` (Host half) plus `lib/client.js` (browser half).
