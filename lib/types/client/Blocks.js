...
import { DocumentFileIcon, IconCheckOutline16, IconCopyOutline16, JsonBlock, fileSizeText, projectUserText, writeClipboard, } from '@deepseek-ai/dsh-client-ui-primitives';
...
export function UserText({ text, sessionLabels, slashNames }) {
    let projected = null;
    try {
        if (typeof projectUserText === 'function')
            projected = projectUserText(text, sessionLabels ?? [], slashNames ?? []);
    }
    catch {
        projected = null;
    }
    return _jsx(_Fragment, { children: projected ?? text });
}
/** One sent file, as the native chat cards it: name over extension and size. */
export function FileCard({ attachment }) {
    ...
    const extension = dot > 0 && dot < attachment.name.length - 1
        ...
        : '';
    return _jsxs("span", { className: css.fileCard, title: attachment.name, children: [_jsx(DocumentFileIcon, { className: css.fileIcon }), _jsxs("span", { className: css.fileContent, children: [_jsx("span", { className: css.fileName, children: attachment.name }), _jsx("span", { className: css.fileMeta, children: [extension, fileSizeText(attachment.bytes)].filter(Boolean).join(' ') })] })] });
}
...
function fallback(block, streaming, source, loadImage, holdFormatting, presentation) {
    switch (block.kind) {
        case 'text': return source === 'user' ? _jsx(UserText, { text: block.text }) : _jsx(ReadingMarkdown, { text: block.text, streaming: streaming, holdFormatting: holdFormatting, ...presentation });
...
export function UserMessageCopy({ text, time }) {
    ...
    return _jsxs("div", { className: css.userActions, children: [timeText && _jsx("span", { className: css.userTime, children: timeText }), _jsx("button", { type: "button", className: css.userCopy, "aria-label": copied ? '已复制' : '复制', title: copied ? '已复制' : '复制', onClick: onCopy, children: copied ? _jsx(IconCheckOutline16, {}) : _jsx(IconCopyOutline16, {}) })] });
}
