...
const FlowSnapshot = createContext(null);
/** Public projection only. Descendants cannot leak live data through a frozen frame. */
export const useFlowChat = selector => {
    const snapshot = useContext(FlowSnapshot);
    if (!snapshot)
        throw new Error('Flow snapshot provider missing');
    return selector(snapshot);
};
const idle = (frame) => ({ phase: 'idle', shown: frame, target: frame, retiring: [], beforeKeys: new Set() });
function Summary({ item, open, onChange, motion }) {
    const button = useRef(null);
    return _jsx("div", { "data-reader-live-fold": true, "data-expanded": open, "data-reader-live-fold-summary": item.summary, children: _jsxs("div", { className: css.summaryRow, children: [_jsx(Disclosure, { open: open, onChange: onChange, buttonRef: button, ariaLabel: "\u6B64\u524D\u6B65\u9AA4", label: _jsx(FoldSummaryText, { summary: item.summary, motion: motion }) }), _jsx(DiffStat, { steps: item.steps, label: item.summary })] }) });
}
...
function FlowCell({ hidden, instant, motion, rowKey, children, summary = false }) {
    ...
    return _jsx("div", { ref: ref, className: css.flowCell, "data-flow-key": rowKey, "data-flow-summary": summary || undefined, hidden: hidden && !present, "aria-hidden": hidden || undefined, ...(hidden ? { inert: '' } : {}), children: present && _jsx("div", { className: css.flowCellInner, children: children }) });
}
export function ChoreographedFlow({ frame, motion, enabled, urgent, open, onOpenChange, processOpen, renderStep, id }) {
    ...
    const [state, setState] = useState(() => idle(frame));
    ...
    useEffect(() => {
        if (state.phase !== 'count' && state.phase !== 'settle' && state.phase !== 'reveal')
            return;
    ...
    useLayoutEffect(() => {
        if (state.phase !== 'reveal' || !root.current)
            return;
        const el = root.current;
        el.dispatchEvent(new CustomEvent('reader-layout-start', { bubbles: true }));
        ...
        const animations = Array.from(el.children).filter(child => !child.hidden && !state.beforeKeys.has(child.dataset.flowKey))
    ...
    return _jsx(FlowSnapshot.Provider, { value: source.snapshot, children: _jsx(StreamMotionContext.Provider, { value: { ...stream, paused: state.phase !== 'idle', resumed: state.phase === 'reveal' }, children: _jsx("div", { id: id, ref: root, className: css.choreographedFlow, "data-reader-flow": true, "data-reader-transition": state.phase, "data-ud-motion": "fold-choreography", children: rows.map(row => {
                    if (row.kind === 'summary')
                        return _jsx(FlowCell, { rowKey: row.key, hidden: false, instant: true, motion: motion, summary: true, children: _jsx(Summary, { item: row.item, open: processOpen && !!open[row.key], onChange: value => onOpenChange(row.key, value), motion: motion && state.phase !== 'collapse' }) }, row.key);
                    ...
                    return _jsx(FlowCell, { rowKey: row.key, hidden: hidden, instant: state.phase !== 'idle', motion: motion, children: renderStep(row.step, !!row.foldKey) }, row.key);
                }) }) }) });
}
//# sourceMappingURL=ChoreographedFlow.js.map