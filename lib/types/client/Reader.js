...
const MainNode = memo(function MainNode({ useChat, nodeKey, boundary, pinned, processOpen = false, ...render }) {
    ...
    if (isNode(node, 'user') || isNode(node, 'steering')) {
        ...
        return _jsxs("div", { className: css.userGroup, "data-reader-anchor": true, "data-reader-key": nodeKey, children: [node.kind === 'steering' && _jsx("p", { className: css.meta, children: "\u8865\u5145\u6D88\u606F" }), _jsx("div", { className: css.user, children: _jsx(Blocks, { ...render, blocks: userBlocks, source: "user" }) }), _jsx(UserMessageCopy, { time: node.data.time, blocks: userBlocks })] });
    }
