...
const MainNode = memo(function MainNode({ useChat, nodeKey, boundary, pinned, processOpen = false, ...render }) {
    ...
    if (isNode(node, 'user') || isNode(node, 'steering')) {
        ...
        return _jsxs("div", { className: css.userGroup, "data-reader-anchor": true, "data-reader-key": nodeKey, children: [node.kind === 'steering' && _jsx("p", { className: css.meta, children: "\u8865\u5145\u6D88\u606F" }), attached > 0 && _jsxs("div", { className: css.userAttachments, "data-reader-attachments": true, children: [parts.images.map(image => _jsx(BlockBoundary, { children: _jsx(ImageBlock, { attachment: image, loadImage: render.loadImage, compact: attached > 1 }) }, `image:${image.attachmentId}`)), parts.files.map(file => _jsx(BlockBoundary, { children: _jsx(FileCard, { attachment: file }) }, `file:${file.attachmentId}`))] }), (parts.text !== '' || parts.rest.length > 0) && _jsxs("div", { className: css.user, children: [_jsx(UserText, { text: parts.text, sessionLabels: data.referenceLabels, slashNames: data.skillNames }), parts.rest.map((block, index) => _jsx(BlockBoundary, { children: _jsx(JsonBlock, { label: "\u9644\u52A0\u5185\u5BB9", payload: block, truncatedLabel: truncatedJsonLabel }) }, `extra:${index}`))] }), _jsx(UserMessageCopy, { time: data.time, text: parts.text })] });
    }
    ...
    if (isNode(node, 'model-retry'))
        return _jsx(RetryItem, { node: node.data.current });
    ...
    if (node.kind === 'context' || node.kind === 'turn-tail')
        return null;
    if (isNode(node, 'system-prompt'))
        return _jsxs("details", { className: css.detail, "data-reader-anchor": true, children: [_jsx("summary", { children: "\u7CFB\u7EDF\u63D0\u793A\u8BCD" }), _jsx("pre", { className: css.promptText, children: node.data.text })] });
...
export function Reader(props) {
    ...
    const turnItems = props.useChat(snapshot => snapshot.navigation.items());
    const [activeTurn, setActiveTurn] = useState(null);
    const navigateTurn = useCallback((item) => {
        const cc = root.current?.closest('[data-conversation-scroll]') ?? null;
        if (!cc)
            return;
        const scrollTo = (key) => {
            const row = cc.querySelector(`[data-reader-key="${CSS.escape(key)}"]`);
            if (row) {
