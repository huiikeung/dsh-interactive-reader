...
const MainNode = memo(function MainNode({ useChat, nodeKey, boundary, pinned, processOpen = false, ...render }) {
    ...
    return _jsx(OfficialNode, { renderSlot: render.renderSlot, node: node, cwd: undefined, openFile: render.openFile, inspectCall: undefined, forkAt: render.forkAt, loadImage: render.officialImageLoader, officialFileMentions: render.officialFileMentions, fallback: _jsxs("div", { className: css.unknown, "data-reader-anchor": true, children: [_jsxs("p", { children: ["\u6B64\u8BB0\u5F55\u7C7B\u578B\u6682\u672A\u63A5\u5165\u9605\u8BFB\u9875\uFF1A", node.kind] }), _jsx(JsonBlock, { label: "\u67E5\u770B\u539F\u59CB\u8BB0\u5F55", payload: node.data, truncatedLabel: truncatedJsonLabel })] }) });
});
/**
...
const TurnGroup = memo(function TurnGroup({ group, motion, autoFold, pinnedKeys, selectedProcessKeys, isAwaitingModel = false, ...props }) {
    ...
    const producedMentions = useMemo(() => deliverables.length > 0 && props.openFile ? createProducedFileMentions(deliverables, props.openFile) : undefined, [deliverables, props.openFile]);
    ...
    const fileMentions = useMemo(() => composeFileMentions(officialMentions, producedMentions), [officialMentions, producedMentions]);
...
export function Reader(props) {
    ...
    return _jsx(StreamMotionContext.Provider, { value: streamMotion, children: _jsxs("div", { ref: root, className: css.root, "data-dsh-interactive-reader": "1.0.0", "data-reader-wait-clock-version": "input-v1", "data-reader-wait-start": waitAnchor.time ?? undefined, "data-motion": motion ? 'on' : 'off', "data-reader-glass": frostedGlass || undefined, "data-reader-auto-fold": autoFold ? 'on' : 'off', children: [_jsx(TimelineRail, { items: timelineItems, activeTurn: activeTurn, busyTurn: busyTurn, onNavigate: onNavigateTurn }), _jsxs("div", { className: css.column, "data-chat-flow": "", children: [_jsxs(StickyLane, { kind: "toolbar", className: css.toolbar, children: [_jsx("button", { type: "button", className: css.textButton, "aria-pressed": autoFold, onClick: () => {
