...
const ICONS = { write: IconEditOutline16, read: IconBrowseOutline16, terminal: IconApiOutline14, search: IconSearchOutline16, web: IconSearchOutline16, code: IconApiOutline14, other: IconSparkle16 };
...
function CustomToolWrapper({ Component, block, toolName, cwd, openFile }) {
    ...
    useEffect(() => {
        ...
        const row = containerRef.current?.querySelector('[role="button"][aria-expanded="false"]');
        if (row)
            row.click();
    }, []);
    return (_jsx("div", { ref: containerRef, "data-reader-tool-custom": true, children: _jsx(Component, { block: block, toolName: toolName, cwd: cwd, openFile: openFile }) }));
}
function ResultView({ entry, model, phase, ...render }) {
    ...
    if (CustomToolView) {
        return _jsx(OfficialTool, { renderSlot: render.renderSlot, block: block, toolName: model.name, cwd: model.cwd, loadImage: render.officialImageLoader, openFile: render.openFile, fallback: _jsx(CustomToolWrapper, { Component: CustomToolView, block: block, toolName: model.name, cwd: model.cwd, openFile: render.openFile }) });
    }
