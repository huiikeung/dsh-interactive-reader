...
async function openWorkspacePath(ctx, path, action) {
    ...
    if (!remoteSession?.openWorkspacePath) {
        console.warn('[dsh-interactive-reader] remote.session is not available');
        return false;
    }
    const result = await remoteSession.openWorkspacePath(action === undefined ? { path } : { path, action });
    if (!result?.ok) {
        console.warn('[dsh-interactive-reader] openWorkspacePath failed:', result?.error?.message);
        return false;
    }
    return true;
}
...
function openFolderPane(ctx, folder) {
    ...
    try {
        ...
        console.warn('[dsh-interactive-reader] opening the folder pane failed:', error);
        return false;
    }
}
...
export function apply(ctx) {
    ...
    pinNavGlyph(['交互阅读', 'Interactive Reader'], 'data-interactive-reader-nav-icon', navGlyph);
                ...
                return {
                    ...
                    openFile: async (path) => {
                        try {
                            ...
                            console.warn('[dsh-interactive-reader] openFile error:', error);
                        }
                    },
                    revealFile: async (path) => {
                        try {
                            ...
                            console.warn('[dsh-interactive-reader] revealFile error:', error);
                            return 'failed';
                        }
                    },
                    forkAt: (seq) => {
                        ...
                        if (typeof seq !== 'number' || !Number.isFinite(seq)) {
                            console.warn('[dsh-interactive-reader] fork refused: missing anchor seq');
                            return;
                        }
                        try {
                            ...
                            if (sessionsApi?.fork) {
                                sessionsApi.fork({ sessionId, atSeq: seq, increaseTitle: true })
                                    ...
                                    .catch(err => { console.warn('[dsh-interactive-reader] fork failed:', err); });
                            }
                        }
                        catch (error) {
                            console.warn('[dsh-interactive-reader] forkAt error:', error);
                        }
                    },
                    loadThrough: async (seq) => {
                        try {
                            ...
                            console.warn('[dsh-interactive-reader] loadThrough error:', error);
                        }
                    },
                    fillComposer: (text) => {
