 ...
 */
export function findComposerTextarea(doc) {
    ...
    if (areas.length === 0)
        return null;
    const visible = areas.filter((el) => !el.disabled && el.offsetParent !== null);
    ...
    for (const el of candidates) {
        if ((el.offsetHeight || 0) > (best.offsetHeight || 0))
            best = el;
    }
    return best;
}
 ...
 */
export function fillComposerDom(text, doc) {
    const d = doc ?? (typeof document !== 'undefined' ? document : undefined);
    if (!d)
        return false;
    try {
        const surfaces = Array.from(d.querySelectorAll('[data-composer-input]'));
        const editable = surfaces.find((el) => el.getAttribute('contenteditable') === 'true');
        const target = editable ?? surfaces[0];
        if (!target)
            return false;
        target.focus();
        if (typeof d.execCommand === 'function') {
            try {
                if (d.execCommand('insertText', false, text))
                    return true;
            }
            catch {
