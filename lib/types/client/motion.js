...
export function useReadingScroll(root, motion) {
    ...
    useLayoutEffect(() => {
        ...
        const capture = () => {
            ...
            const candidates = content.querySelectorAll('[data-reader-anchor]');
            let candidate = null;
        ...
        const onScroll = () => {
            ...
            if (followFrame !== 0 || layoutDepth > 0)
                return;
        ...
        const follow = (now) => {
            followFrame = 0;
            if (!following.current || selected() || editingText())
                return;
            if (layoutDepth > 0)
                return;
