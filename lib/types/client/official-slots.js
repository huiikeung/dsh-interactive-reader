...
export const officialSeat = (family, source = FAMILIES[family]) => `dsh-interactive-reader.official.${family}/${source}`;
/**
 ...
 */
export const READER_NODES = new Set([
 ...
 */
export function producedPathTailMatch(value, displayedPaths) {
...
function tailPresentation(component) {
    ...
    return memo(function ReaderTailPresentation(props) {
        const matched = useMemo(() => producedPathTailMatch(props.matched, props.readerProducedPaths), [props.matched, props.readerProducedPaths]);
    ...
    for (const name of Object.keys(FAMILIES)) {
        ...
        try {
            disposers.push(slots.inject(source, () => {
                ...
                return mirrorOfficialSlot(slots, source, officialSeat(family), `dsh-interactive-reader.official.${family}`, family === 'nodes' ? entry => !READER_NODES.has(entry.options.key ?? '') : undefined);
            }));
        }
        catch (error) {
