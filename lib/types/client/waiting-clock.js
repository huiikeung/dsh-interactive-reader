 ...
 */
export const WAIT_AFTER = new Set(['context', 'model-retry']);
/**
 ...
 */
function toolSettled(data) {
    const root = data?.root;
    return typeof root === 'object' && root !== null && root.kind === 'tool-result';
}
/** A command that is still executing has no outcome yet; only a settled one hands over. */
function commandSettled(data) {
 ...
 * the model can stall.
 */
export function handsBackToModel(node) {
    if (node.kind === 'user' || node.kind === 'steering')
        return true;
    if (node.kind === 'tool-call')
        return toolSettled(node.data);
    if (node.kind === 'command')
        return commandSettled(node.data);
    return WAIT_AFTER.has(node.kind);
}
/**
