...
export const FOLDER_TAB_ID = 'dsh-interactive-reader.folder';
export const FOLDER_TAB_KIND = 'interactive-reader-folder';
/** The one scheme the shell's tab service accepts. */
export const FOLDER_ADDRESS_PREFIX = 'dsh-resource://interactive-reader-folder/';

/** The address that opens one folder in this pane. */
export function folderAddressOf(absolutePath: string): string {
...
export function folderPathOf(address: string): string | null {
  if (typeof address !== 'string' || !address.startsWith(FOLDER_ADDRESS_PREFIX)) return null;
  try {
...
export function folderTabTitle(address: string): string {
  const path = folderPathOf(address);
  if (path === null || path === '') return '文件夹';
...
export function sortFolderEntries(entries: readonly FolderEntry[]): FolderEntry[] {
  const rank = (entry: FolderEntry): number => (entry.type === 'directory' ? 0 : entry.type === 'file' ? 1 : 2);
  return [...entries].sort((left, right) =>
    rank(left) - rank(right) || left.name.localeCompare(right.name, undefined, { numeric: true }));
}
...
export function parentFolderOf(path: string): string | null {
...
export const folderTabDefinition = {
  ...
  patterns: ['dsh-resource://interactive-reader-folder/**'],
  ...
  canOpen: (address: string): boolean => folderPathOf(address) !== null,
