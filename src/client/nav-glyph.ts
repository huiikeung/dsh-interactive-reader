...
export function navGlyph(): NavGlyphSpec {
  return {
    viewBox: '0 0 16 16',
 ...
 */
export function pinNavGlyph(labels: readonly string[], mark: string, glyph: () => NavGlyphSpec): void {
  if (typeof document === 'undefined' || typeof MutationObserver === 'undefined') return;
  const applyGlyph = (): void => {
    ...
    for (const cell of Array.from(document.querySelectorAll('[role="dialog"] nav button'))) {
      ...
      let spec: NavGlyphSpec;
      try {
        spec = glyph();
      } catch (error) {
        console.warn('[dsh-interactive-reader] nav glyph failed; keeping the shell icon', error);
        continue;
      }
