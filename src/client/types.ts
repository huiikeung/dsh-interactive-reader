...
// Namespace type imports, not `import type {}`: an empty type-only import is
// dropped before the module resolves, so the slot augmentations these packages
// contribute (`useChat`, `useConversation`, `useSession`, `sessionId`, …) never
// enter this program and `PropsRuntime<'conversation.view'>` loses its session
// half. Naming the namespace keeps the declaration file loaded and emits nothing.
...
export type BlockRenderProps = Pick<ReaderProps, 'renderSlotChain' | 'loadImage' | 'fillComposer'> & {
  ...
  revealFile?: (path: string) => Promise<void> | void;
  forkAt?: (seq: number) => void;
  fileMentions?: Record<string, { open: () => void; label: string; title?: string }>;
  metrics?: {
    usage?: {
