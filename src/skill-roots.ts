...
export function resolveDshHome(env: Record<string, string | undefined> = process.env, home = homedir()): string {
...
export function resolveAgentsHome(env: Record<string, string | undefined> = process.env, home = homedir()): string {
...
export function projectSkillRoots(projectRoot: string): readonly SkillRootInfo[] {
  return [
...
export async function rootHasGenerativeMcpapps(rootPath: string): Promise<boolean> {
  let entries;
  try {
...
export function pluginSkillPackPath(moduleUrl = import.meta.url): string | undefined {
...
export async function scanGenerativeMcpappsStatus(options: {
  ...
  home?: string;
  packPath?: string;
  listSkills?: () => Promise<readonly { readonly name?: string }[] | undefined>;
} = {}): Promise<HostSkillStatus> {
  const env = options.env ?? process.env;
  const home = options.home ?? homedir();
  const roots: SkillRootInfo[] = [...userSkillRoots(env, home)];
  ...
  let via: HostSkillStatus['via'] = null;
  let listHit = false;
